...
//this works because of closure e can be accessed when it finally runs
button.addEventListener("click",function(e){
    e.preventDefault()//
});

function alertUser(name){
    // this works because of closure name can be accessed when it finally runs
     aler("Welcome " + name + " ,you are logged in.");
     }
     singUp(callback){
         // do something here
         callback();
    }
singUp(alertUser(name);
```
